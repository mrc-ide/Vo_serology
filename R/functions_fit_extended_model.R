################################################################################
# Moment generating function of infectiousness for extended model,             #
# assuming gamma distributed hazard.                                           #   
################################################################################
MGF_gamma <- function(x, n, beta, alpha, shape, z){
  
  if(is.infinite(shape)){ 
    
    return(exp((-beta/((n - z)^alpha))*(x)))
    
  }else{
      
      return((shape/(shape+x*(beta/((n - z)^alpha))))^shape)
  ...
  for(i in 1:(s0+1)){
    ...
    for(j in 1:(s0+1)){
      ...
      if(i <= j){
        ...
        den <- ((MGF_gamma((s0-k), n, beta, alpha, shape, z))^m)*Q^(s0-k)
