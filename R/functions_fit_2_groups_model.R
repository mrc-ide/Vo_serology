################################################################################
# Moment generating function of infectiousness for 2-groups model              #   
################################################################################
MGF_gamma <- function(x, n, beta_high, alpha, beta_low, p_high){
      
    return(p_high * exp((-beta_high/((n)^alpha))*(x)) + 
             ((1-p_high) * exp((-beta_low/((n)^alpha))*(x))))      
}
...
compute_F <- function(n, s0, Q, parameters){
  ...
  beta_high <- parameters[1]
  alpha <- parameters[3]
  beta_low <- parameters[8]
  p_high <- parameters[9]
  ...
  for(i in 1:(s0+1)){
    ...
    for(j in 1:(s0+1)){
      ...
      if(i <= j){
        ...
        den <- (as.numeric((MGF_gamma((s0-k), n, beta_high, alpha, beta_low, p_high))^m)*Q^(s0-k))
...
compute_R <- function(data, parameters, r){
  ...
  Q <- unlist(parameters[2])
  Q_prior <- unlist(parameters[4])
  p_pr <- unlist(parameters[5])
...
deviance <- function(data, parameters){
  ...
  one_minus_pcom_mat[1,] <- 1-parameters[7]
  model <- one_minus_pcom_mat + mod*parameters[7]
...
run_MCMC <- function(data, mcmc_par, parameters, parametersToEstimate){
  ...
  estimate_beta_low <- FALSE
  idx_beta_low <- 0
  if(8 %in% which(parametersToEstimate > 0)){
    estimate_beta_low <- TRUE
    idx_beta_low <- which(which(parametersToEstimate > 0) == 8) 
  }
  
  if(any(currentParVector == 0)){
    ...
    for(parID in 1:length(currentParVector)){ 
      ...
      if(log(runif(1)) < Nu){  # acceptance rule
        ...
        if(idx_beta_low != 0){
          ...
          if(parID == idx_beta_low){
            
            if(newValue < currentParVector[1]){
            ...
            if(newValue > currentParVector[idx_beta_low]){
...
wrapper_model <- function(model_variant, 
  ...
  parametersToEstimate <- unlist(par[model_variant, 2:10])
