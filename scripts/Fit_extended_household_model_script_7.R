################################################################################
# This code implements an extension of the method described in Fraser et al    #
# (2011), Am J Epidemiol (https://doi.org/10.1093/aje/kwr122). See Methods for #
# details on the extension. Here we fit the model to the SARS-CoV-2 household  #
# data collected in Vo'.                                                       #
#                                                                              #
# Cite as:                                                                     #
...
# source scripts
source("R/functions_fit_extended_model.R")
...
#
# # load sources
# list_sources <- c("R/functions_fit_extended_model.R",
...
parameters <- read.csv("data/household_model/Default_parameters_extended.csv",
                       header = TRUE)

# read model variants
par <- read.csv("data/household_model/Model_variants_extended.csv",
                header = TRUE)

# read MCMC parameters
mcmc_par <- read.csv("data/household_model/MCMC_parameters_extended.csv",
                     header = TRUE)

# Create output folders -------------------------------------------------------#
dir_output  <- file.path("mcmc_posterior_chains_extended")
...
file_res <- "mcmc_posterior_estimates_extended.csv"
...
for(j in 1:length(res)){
  ...
  for(i in 1:length(res[[j]][[1]])){
    ...
    parametersToEstimate <- unlist(par[j, 2:10])
