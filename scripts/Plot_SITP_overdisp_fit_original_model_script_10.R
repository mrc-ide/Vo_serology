...
j <- 1 # initialise
N <- 100 # number of draws from posterior distribution 
res <- list()
...
df_AR_final <- data.frame(value = c(AR,
                          ...
                          low = c(AR_low,
                                  ...
                                  SAR_low,
                                  apply(SAR_mat, 2,quantile,0.025,na.rm = TRUE),
                                  house_AR_ci_low,
                                  house_AR_exp_ci[1,]),
                          ...
                          up = c(AR_up,
                                 ...
                                 SAR_up,
                                 apply(SAR_mat, 2,quantile,0.975, na.rm = TRUE),
                                 house_AR_ci_up,
                                 house_AR_exp_ci[2,]),
...
#df_SITP_P <- df_SITP[which(df_SITP$model %in% selected_models[idx]), ]
...
for(j in 1:length(df_overdisp)){
  ...
  df_mu[[j]][,1] <- df_overdisp[[j]][indexes,1]
  df_mu[[j]][,2] <- df_overdisp[[j]][indexes,2]
  
  df_overdisp_low[[j]] <- df_overdisp[[j]][which(df_overdisp[[j]][,6]==lim[1]),]
  df_overdisp_up[[j]] <- df_overdisp[[j]][which(df_overdisp[[j]][,6]==lim[2]),]
...
plot_SITP_final <- ggplot(df_SITP_selected) +
  geom_errorbar(aes(x= size, 
                ...
                width= 0, 
                size = 0.7, 
                colour = rep(c(cols[ncol[1]], cols[ncol[3]]), 6))+ 
